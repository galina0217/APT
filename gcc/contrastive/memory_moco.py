...
class MemoryMoCo(nn.Module):
    ...
    def forward(self, q, k, update=True, ret_max=False):
        batchSize = q.shape[0]
        k = k.detach()
        ...
        max_neg = torch.tensor([max(l_neg.tolist()[i]) for i in range(len(l_neg))]).view(1, -1).tolist()
        ...
        if ret_max == True:
            return l_pos.view(1, -1).tolist(), max_neg
        else:
            return out
